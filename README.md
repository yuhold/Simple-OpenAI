...
cd plugins
git clone https://github.com/你的用户名/Simple-OpenAI.git
...
Q: 发送消息后没有任何反应？ A:
    ...
    如果是私聊，请检查是否需要加 #chat 前缀（除非开启了免前缀模式）。

    检查是否在黑名单中。

    检查是否开启了白名单模式且自己不在名单内。

Q: 报错 fetch failed 或 ConnectTimeoutError？ A: 这是网络问题。

    如果使用官方 API，请在锅巴里填写 HTTP代理地址 (如 http://127.0.0.1:7890)。

    如果使用国内中转，请开启 “使用自定义API地址” 并检查地址是否正确。

    确保你执行了 pnpm install 安装了 https-proxy-agent。

Q: AI 回复乱码或带有很多符号？ A: 请在锅巴配置中开启 “去除Markdown格式” 开关。
📝 更新日志
